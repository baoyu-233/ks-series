# ============================================================
# ks-Eco-tax Auto Build & Deploy Script
...
$jarName = "ks-Eco-tax-$version.jar"
$jarPath = Join-Path $ProjectRoot "target\$jarName"

Write-Host "============================================" -ForegroundColor Cyan
Write-Host " ks-Eco-tax Build & Deploy v$version" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
...
if (-not $NoBackup) {
    ...
    Get-ChildItem $TestServerPlugins "ks-Eco-tax-*.jar" -ErrorAction SilentlyContinue | ForEach-Object {
        $ts = Get-Date -Format 'yyyyMMdd-HHmmss'
        Copy-Item $_.FullName (Join-Path $ArchivesDir "ks-Eco-tax-backup-$ts-$($_.Name)")
...
$ServerConfigDir = Join-Path $TestServerPlugins "ks-Eco-tax"
...
if (Test-Path $SrcConfig) {
    ...
    if ((Test-Path $ServerConfigFile) -and (-not $NoBackup)) {
        Copy-Item $ServerConfigFile (Join-Path $ArchivesDir "ks-Eco-tax-config-backup-$(Get-Date -Format 'yyyyMMdd-HHmmss')-config.yml")
    }

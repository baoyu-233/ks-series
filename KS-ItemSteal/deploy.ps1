# ============================================================
# KS-ItemSteal Auto Build & Deploy Script
...
$jarName = "KS-ItemSteal-$version.jar"
$jarPath = Join-Path $ProjectRoot "target\$jarName"

Write-Host "============================================" -ForegroundColor Cyan
Write-Host " KS-ItemSteal Build & Deploy v$version" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
...
if (-not $NoBackup) {
    ...
    Get-ChildItem $TestServerPlugins "KS-ItemSteal*.jar" -ErrorAction SilentlyContinue | ForEach-Object {
        $ts = Get-Date -Format 'yyyyMMdd-HHmmss'
        Copy-Item $_.FullName (Join-Path $ArchivesDir "KS-ItemSteal-backup-$ts-$($_.Name)")
...
# Legacy cleanup
Get-ChildItem $TestServerPlugins "ItemSteal.jar" -ErrorAction SilentlyContinue | ForEach-Object {
    if (-not $NoBackup) { Copy-Item $_.FullName (Join-Path $ArchivesDir "legacy-$($_.Name)") }
    Remove-Item $_.FullName -Force; Write-Host "[Deploy] Removed legacy: $($_.Name)" -ForegroundColor Yellow
}
Write-Host "[Deploy] OK - $jarName" -ForegroundColor Green

$SrcConfig = Join-Path $ProjectRoot "src\main\resources\config.yml"
$ServerConfigDir = Join-Path $TestServerPlugins "KS-ItemSteal"
...
if (Test-Path $SrcConfig) {
    ...
    if ((Test-Path $ServerConfigFile) -and (-not $NoBackup)) {
        Copy-Item $ServerConfigFile (Join-Path $ArchivesDir "KS-ItemSteal-config-backup-$(Get-Date -Format 'yyyyMMdd-HHmmss')-config.yml")
    }

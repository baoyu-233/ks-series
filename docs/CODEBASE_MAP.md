...
跨服地图、房产和资产只读投影位于 `crossserver/assets/`。`KsEco` 负责严格解析 `federated-assets`、异步建表、
节点心跳、关闭取消、原子热加载和 database-lane Future 门面；Extra 只能提交已从本地 Bukkit 状态截取并冻结的
`PreparedSnapshot`，repository/publisher 不持有也不远程读取 `World`。`EcoWebHandler` 暴露
`/api/federated/snapshot-sources|snapshot|assets|assets/aggregate` 和管理员 settings API。MAP、PROPERTY、ASSET
查询都先执行 server/world/dimension 默认拒绝策略，deny 优先；PROPERTY 资产还必须同时通过 property-trade
能力，避免只开启资产汇总时泄露房产信息。共享数据库连接、`database.server-id` 与 transport 为重启边界。
实际生产者目前是 `ksHWP/FederatedMapPublisher` 与 `ks-Eco-RealEstate/FederatedPropertyPublisher`：前者按来源维护
有界多图块 bundle，缓存命中也必须重新进入逐图块节流发布；后者按来源聚合全部 READY 房屋并用单调 revision
防止旧异步结果覆盖。READY DTO 的可空字段由防御性 `LinkedHashMap` 冻结，不能使用拒绝 null 的 `Map.copyOf`。
...
on the Paper 主线程或 Folia global owner；their futures complete on the corresponding owner thread. Cached snapshot/grid
queries are immutable and safe for Web readers. `registerSchematicRoot`/`unregisterSchematicRoot` 只修改并发路径表，
显式允许 Extra 启用线程调用，不能复用 prepare/release 的 global-thread guard。
...
`internal/SchematicRepository.java`. WorldEdit/FAWE 通过 `SchematicRuntime`/`OptionalSchematicRuntime` 可选适配器
延迟反射加载；依赖缺失时插件/API 仍启用，仅 schematic prepare/cleanup 在功能级失败关闭。服务器 owner 线程的
canvas 操作和 tick-bounded Bukkit marker scans 位于 `internal/CanvasService.java` 与 `internal/MarkerScanner.java`。
...
递归合成重新放进 Paper 主线程。`FederatedMapPublisher` 只接收冻结 PNG，通过反射窄桥发布到 ks-Eco；同一来源
使用 `ks-hwp-map-tile-bundle/v1` 有界滚动集合，超过上限淘汰最旧图块。`renderTileAsync` 命中内存缓存时仍要
调用发布器，否则插件重启后的空 bundle 无法随实际浏览恢复。
...
`network_1_21/roles.yml`。Leaves `test_1_21` 是生存主端与 Web 8123；Paper RPG 按角色禁用 Web；Folia 实验端
为同口径验收启用仅本机 Web 58578。三个 ks-Eco 节点 ID 分别为 `survival`、`rpg`、`folia`，共同使用工作区外
MariaDB 与 JDBC 内置经济。Leaves/Folia 各 29 项 Web 合同返回预期状态；Extra 数量分别为 6/4/6。

Folia 构建仍必须使用 profile，部署目标统一使用标准文件名；不得同时保留标准名和 `*-folia.jar`，否则 Paper
remapper 会生成同名插件歧义。银行、企业、政治、税务、地产、副本当前可在 Folia 加载，但没有 FAWE/WorldEdit
或 MythicMobs 时对应 schematic/Boss 能力不计为通过。不得把工作区外凭据、MariaDB data、MCSM API key、运行时
配置或测试 token 同步到公开仓库。最终证据位于 `docs/KS-ECO-FULL-FUNCTION-TEST-2026-07-23.md`。

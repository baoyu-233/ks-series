# ============================================================
# ks-Eco-enterprise Auto Build & Deploy Script
...
$jarName = "ks-Eco-enterprise-$version.jar"
$jarPath = Join-Path $ProjectRoot "target\$jarName"

Write-Host "============================================" -ForegroundColor Cyan
Write-Host " ks-Eco-enterprise Build & Deploy v$version" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
...
if (-not $NoBackup) {
    ...
    Get-ChildItem $TestServerPlugins "ks-Eco-enterprise-*.jar" -ErrorAction SilentlyContinue | ForEach-Object {
        $ts = Get-Date -Format 'yyyyMMdd-HHmmss'
        Copy-Item $_.FullName (Join-Path $ArchivesDir "ks-Eco-enterprise-backup-$ts-$($_.Name)")
...
$ServerConfigDir = Join-Path $TestServerPlugins "ks-Eco-enterprise"
...
if (Test-Path $SrcConfig) {
    ...
    if ((Test-Path $ServerConfigFile) -and (-not $NoBackup)) {
        Copy-Item $ServerConfigFile (Join-Path $ArchivesDir "ks-Eco-enterprise-config-backup-$(Get-Date -Format 'yyyyMMdd-HHmmss')-config.yml")
    }

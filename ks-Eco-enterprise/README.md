...
- **Paper/Folia 钱包边界**: 外部 Vault 按收款人的实体/global owner 调度并设有超时取消；Folia 内置钱包使用 ks-Eco 的 UUID 数据库接口，不在区域线程创建或读取 Bukkit 玩家对象
...
本轮模块级验证覆盖旧评标入口的失败关闭边界、企业资金同库事务、个人保证金/预付款 journal、企业房产公户结算、人工复核阶段约束、企业主表、权限/角色模板、分红 journal 的便携 schema/upsert，以及 Folia metadata 契约；本模块当前 4 项测试通过。
...
- Folia 运行要求匹配的 ks-Eco Folia 制品，并由 Extra loader 校验 `folia-supported=true`；本轮未部署、未启动服务器，企业注册、注资、分红与 Web 写操作尚未完成 Folia 实机验收。
- 企业同步写接口当前只供核心 Web/后台工作线程调用；未来 Folia 游戏内 GUI 必须把 JDBC 准备/提交与玩家实体线程上的外部 Vault 结算分段，不能直接在实体线程调用管理器。

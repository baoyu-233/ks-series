# ============================================================
# ksHWP Auto Build & Deploy Script
...
$jarName = "ksHWP-$version.jar"
$jarPath = Join-Path $ProjectRoot "target\$jarName"

Write-Host "============================================" -ForegroundColor Cyan
Write-Host " ksHWP Build & Deploy v$version" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
...
if (-not $NoBackup) {
    ...
    Get-ChildItem $TestServerPlugins "ksHWP-*.jar" -ErrorAction SilentlyContinue | ForEach-Object {
        $ts = Get-Date -Format 'yyyyMMdd-HHmmss'
        Copy-Item $_.FullName (Join-Path $ArchivesDir "ksHWP-backup-$ts-$($_.Name)")
...
$ServerConfigDir = Join-Path $TestServerPlugins "ksHWP"
...
if (Test-Path $SrcConfig) {
    ...
    if ((Test-Path $ServerConfigFile) -and (-not $NoBackup)) {
        Copy-Item $ServerConfigFile (Join-Path $ArchivesDir "ksHWP-config-backup-$(Get-Date -Format 'yyyyMMdd-HHmmss')-config.yml")
    }
